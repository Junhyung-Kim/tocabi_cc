...
CustomController::CustomController(RobotData &rd) : rd_(rd)
{
    ...
    pthread_attr_destroy(&attrs1);
    std::cout <<"THREAD SUCCESS1" << std::endl;
    */
...
void CustomController::setGains()
{
    ...
    kp_stiff_joint_(13) = 6000;
    kp_stiff_joint_(14) = 6000;
    ...
    kv_stiff_joint_(13) = 50;
    kv_stiff_joint_(14) = 50;
            ...
            if (mpc_cycle < controlwalk_time-1)
            {
                    ...
                    {  
                        ...
                        else
                        {
                            ...
                            if(mpc_cycle > 1 || (mpc_cycle == 1 && walking_tick >= 1))
                            {
                                ...
                                del_zmp(0) = 0.2 * (cp_measured_(0) - cp_desired_(0));//1.6, 1.6
                                del_zmp(1) = 0.2* (cp_measured_(1) - cp_desired_(1));
                            }
                        }
                    }
                }
                ...
                */
                    ...
                    if(time_tick_next == true)
                    {
                        if(walking_tick_stop == false || mpc_cycle <= 0)
                        {
                            ...
                            if(rfoot_trajectory_support_.translation()(2) > 0 && contactMode != 1)
                            {
                                ...
                                rfoot_d(0) = rfoot_d(0) + 0.00 * (rfoot_trajectory_support_.translation()(0) - rfoot_support_current_.translation()(0));
                                rfoot_d(1) = rfoot_d(1) + 0.00 * (rfoot_trajectory_support_.translation()(1) - rfoot_support_current_.translation()(1));    
                            }
                            else if(lfoot_trajectory_support_.translation()(2) > 0 && contactMode != 1)
                            {
                                rfoot_d(2) = rfoot_d(2) + 0.00 * (rfoot_trajectory_support_.translation()(2) - rfoot_support_current_.translation()(2)) + F_F_input_dot_d/z_gain;    
                                lfoot_d(2) = lfoot_d(2) + 0.00 * (lfoot_trajectory_support_.translation()(2) - lfoot_support_current_.translation()(2)) - F_F_input_dot_d/z_gain;
                                lfoot_d(0) = lfoot_d(0) + 0.00 * (lfoot_trajectory_support_.translation()(0) - lfoot_support_current_.translation()(0));
                                lfoot_d(1) = lfoot_d(1) + 0.00 * (lfoot_trajectory_support_.translation()(1) - lfoot_support_current_.translation()(1));
                            }
                            else
                            {
                            ...
                            rfoot_ori1(0) = 0.9*rfoot_ori(0) + 0.00 * (rpy_foot(0) - rfoot_rpy_current_(0));
                            rfoot_ori1(1) = 0.9*rfoot_ori(1) + 0.00 * (rpy_foot(1) - rfoot_rpy_current_(1));
                           
                            rpy_foot = DyrosMath::rot2Euler(lfoot_trajectory_float_.linear());
                            lfoot_ori1(0) = 0.9*lfoot_ori(0) + 0.00 * (rpy_foot(0) - lfoot_rpy_current_(0));
                            lfoot_ori1(1) = 0.9*lfoot_ori(1) + 0.00 * (rpy_foot(1) - lfoot_rpy_current_(1));
                           
                           
                            //COM Feedback조심
                       
                            //com_d(2) = 0.0 + 0.003 * (com_desired_init(2) - com_support_current_(2));
                            if(mpc_cycle < 0 || mpc_cycle > 1)
                            {
                            //com_d(0) = com_d(0) + 0.003 * (com_desired_(0) - com_support_current_(0));
                            //com_d(1) = com_d(1)+ 0.003 * (com_desired_(1) - com_support_current_(1));
                            }
                            com_d(2) = 0.0 + 0.003 * (com_desired_init(2) - com_support_current_(2));
                ...
                if(mpc_cycle < controlwalk_time-1)
                {
                    ...
                    file[1] << "12  " << zmpx_ << " " <<  zmp_measured_mj_(0) <<" " <<ZMP_X_REF << " "<<zmpy_ << " " <<  zmp_measured_mj_(1) << " "<<ZMP_Y_REF << " " << zmp_mpcy << " " << desired_val_slow[47]-virtual_temp1(1) << " "<< zmp_mpcx << " " << desired_val_slow[43]-virtual_temp1(0) << " "<< desired_val_slow[43] << " ";
...
{ // 기존 알고리즘에서 바꾼거 : 0. previewcontroller에서 ZMP_Y_REF 변수 추가 1. zmp offset 2. getrobotstate에서 LPF 3. supportToFloatPattern 함수 4. Tau_CP -> 0  5. getfoottrajectory에서 발의 Euler angle
    ...
    if(mpc_cycle >= 0)
    {
        F_F_input_dot = 0.0008 * ((l_ft_(2) - r_ft_(2)) - (F_L - F_R)) - 5.0 * F_F_input; // F_F_input이 크면 다리를 원래대로 빨리줄인다. 이정도 게인 적당한듯0.001/0.00001 // SSP, DSP 게인값 바꿔야?
    ...
    {   //0.1
        if(mpc_cycle > 0)
        {
            F_T_L_x_input_dot = -0.12 * (Tau_L_x - l_ft_LPF(3)) - Kl_roll * F_T_L_x_input;
            F_T_L_x_input = F_T_L_x_input + F_T_L_x_input_dot * del_t;
            //F_T_L_x_input = 0;
            F_T_R_x_input_dot = -0.12 * (Tau_R_x - r_ft_LPF(3)) - Kr_roll * F_T_R_x_input;
            F_T_R_x_input = F_T_R_x_input + F_T_R_x_input_dot * del_t;
            //F_T_R_x_input = 0;

            //Pitch 방향
            F_T_L_y_input_dot = 0.12 * (Tau_L_y - l_ft_LPF(4)) - Kl_pitch * F_T_L_y_input;
            F_T_L_y_input = F_T_L_y_input + F_T_L_y_input_dot * del_t;
            //F_T_L_y_input = 0;
            F_T_R_y_input_dot = 0.12 * (Tau_R_y - r_ft_LPF(4)) - Kr_pitch * F_T_R_y_input;
            F_T_R_y_input = F_T_R_y_input + F_T_R_y_input_dot * del_t;
        }
        else
        {
...
void* CustomController::proc_recv1(){
    ...
    while(true)
    {
        ...
        //if(statemachine_ == 1 || mpc_cycle < 0)
        //    std::cout << "asaa" <<walking_tick_mj << " "<<mpc_cycle << " " <<mpc_start_init_ <<" " << mpc_start_init_bool <<std::endl;
