...
CustomController::CustomController(RobotData &rd) : rd_(rd)
{
    ...
    std::cout << "ZMP_av" << std::endl;
    std::cout << ZMP_bound.size() << std::endl;
    std::cout << "RF_tran_av" << std::endl;
    std::cout << RF_tran.size() << std::endl;
    std::cout << "LF_tran_av" << std::endl;
...
void CustomController::computeSlow()
{
    ...
    if (rd_.tc_.mode == 6)
    {
        ...
        /*file[0] << mpc_cycle << " " << walking_tick << " " << walking_tick_mj << " " << com_support_current_(0)<< " " << com_support_current_(1) << " " << com_desired_(0)<< " " << com_desired_(1)<< " " << zmpx << " "<< zmpx_d << " " << zmp_measured_mj_(0) + 0.03 << " " << zmpy << " "<< zmpy_d << " " << zmp_measured_mj_(1) << " " <<com_alpha<< " "<< qp_result(2) << " " << qp_result(8) << " ";
               
        for (int i = 0; i < 12; i++)
            file[0] << Gravity_MJ_fast_(i) << " " << qdd_des_virtual_(i) << " " ;// << rd_.q_(i) << " " << qd_des_(i)<< " " << rd_.q_dot_(i) << " " << qdd_des_(i) << " ";
        file[0] << std::endl;
        */
    ...
    {
        if(walking_tick_mj >= 3.0 * hz_)
        {
            mpc_cycle = (walking_tick_mj - 6000)/40;
            walking_tick = (walking_tick_mj - 6000)%40;
        }
        ////////////////////////////////////////////////////////////////////////////
        ...
        if (walking_enable_ == true)
        {
            ...
            if (current_step_num_ < total_step_num_)
            {
                ...
                if(walking_tick_mj >= 1)
                {
                    if(momentumControlMode == true)
                    {//3->6으로 확장
                        qdd_des_virtual = (q_dm.head(3) - qd_des_virtual_prev.head(3))*2000;
                        qdd_des_virtual_ori = (q_dm.segment<3>(3) - qd_des_ori_prev) * 2000;
                        qdd_des_.head(12) = (q_dm.segment<12>(6) - qd_des_prev.head(12))*2000;
                        ...
                        qdd_des_virtual_1.head(3) =  qdd_des_virtual.head(3);
                        qdd_des_virtual_1.segment<3>(3) = qdd_des_virtual_ori.head(3);
                        qdd_des_virtual_1.segment<12>(6) =  qdd_des_.head(12);


                        qd_des_virtual_prev.head(3) = q_dm.head(3);
                        qd_des_prev.head(12) = q_dm.segment<12>(6);
                        qd_des_ori_prev.head(3) = q_dm.segment<3>(3);
                    }
                    else
                    {
                        for(int i = 0; i < 12; i++)
                    ...
                    if(momentumControlMode == false)
                    {
                        ...
                        for(int i = 0; i < 12; i ++)
                            q_des_prev(i) = q_des_1(i);
                        for(int i = 0; i < 12; i ++)
                            qd_des_prev(i) = qd_des_lpf(i);//qd_des_lpf(i);
                            //qd_des_prev(i) = rd_.q_dot_(i);//qd_des_lpf(i);//
                    }
                }
...
void CustomController::getRobotState()
{
    ...
    comj = PELV_YaW6D * (rd_.link_[Pelvis].jac.cast<double>());
    ...
    if(walking_tick_mj == 0)
    {
        ...
        com_x_int = com_matrix_matrix(0,0) - com_support_current_init.translation()(0);
        com_y_int = com_matrix_matrix(0,1) - com_support_current_init.translation()(1);
    }
...
void CustomController::getFootTrajectory()
{
    ...
    if (walking_tick_mj < t_start_ + t_rest_init_ + t_double1_)
    {
        ...
        if (foot_step_(current_step_num_, 6) == 1) // 왼발 지지
        {
            ...
            {
                //com_alpha = DyrosMath::cubic(walking_tick_mj, t_start_ + t_rest_init_ + t_rest_temp, t_start_ + t_rest_init_ + t_double1_ + t_rest_temp, 0.5, 0.0, 0.0, 0.0);
            }
        }
        else if (foot_step_(current_step_num_, 6) == 0) // 오른발 지지
        {
            ...
            if(walking_tick_mj > t_start_ + t_rest_init_  + t_rest_temp)
            {
                //com_alpha = DyrosMath::cubic(walking_tick_mj, t_start_ + t_rest_init_ + t_rest_temp, t_start_ + t_rest_init_ + t_double1_ + t_rest_temp, 0.5, 1.0, 0.0, 0.0);
            }
        }
    ...
    else
    {
        ...
        if (foot_step_(current_step_num_, 6) == 1)
        {
            //com_alpha = DyrosMath::cubic(walking_tick_mj, t_start_ + t_total_ - t_double2_ - t_rest_last_, t_start_ + t_total_ - t_rest_last_, 0.0, 0.5, 0.0, 0.0);
            lfoot_trajectory_euler_support_.setZero();
            //lfoot_trajectory_support_.linear() = DyrosMath::rotateWithZ(lfoot_trajectory_euler_support_(2)) * DyrosMath::rotateWithY(lfoot_trajectory_euler_support_(1)) * DyrosMath::rotateWithX(lfoot_trajectory_euler_support_(0));
            lfoot_trajectory_support_.linear() = DyrosMath::rotateWithZ(lfoot_trajectory_euler_support_(2)) * DyrosMath::rotateWithY(F_T_L_y_input) * DyrosMath::rotateWithX(-F_T_L_x_input);
        ...
        {
            //com_alpha = DyrosMath::cubic(walking_tick_mj, t_start_ + t_total_ - t_double2_ - t_rest_last_, t_start_ + t_total_ - t_rest_last_, 1.0, 0.5, 0.0, 0.0);
            rfoot_trajectory_euler_support_.setZero();
            rfoot_trajectory_support_.linear() = DyrosMath::rotateWithZ(rfoot_trajectory_euler_support_(2)) * DyrosMath::rotateWithY(F_T_R_y_input) * DyrosMath::rotateWithX(-F_T_R_x_input);
...
void CustomController::getPelvTrajectory()
{
    ...
    /*if(mpc_cycle == 0)
    {
        com_desired_(0) = com_matrix_matrix(mpc_cycle, 0) - com_x_int;
        com_desired_(1) = com_matrix_matrix(mpc_cycle, 1) - com_y_int;
    }
    else
    {
        com_desired_(0) = (com_matrix_matrix(mpc_cycle+1, 0) * walking_tick + com_matrix_matrix(mpc_cycle,0) * (40 -walking_tick))/40  - com_x_int;
        com_desired_(1) = (com_matrix_matrix(mpc_cycle+1, 1) * walking_tick + com_matrix_matrix(mpc_cycle,1) * (40 -walking_tick))/40  - com_y_int;
    }*/
...
void CustomController::CP_compen_MJ()
{
    ...
    else
    {
        ...
        F_R = alpha * rd_.link_[COM_id].mass * GRAVITY;
        F_L = (1 - alpha) * rd_.link_[COM_id].mass * GRAVITY;
...
{ // 기존 알고리즘에서 바꾼거 : 0. previewcontroller에서 ZMP_Y_REF 변수 추가 1. zmp offset 2. getrobotstate에서 LPF 3. supportToFloatPattern 함수 4. Tau_CP -> 0  5. getfoottrajectory에서 발의 Euler angle
    ...
    else
    {
        ...
        /*Tau_R_x = (alpha) * Tau_all_x;
        Tau_L_x = (1-alpha) * Tau_all_x;

        Tau_R_y = -(alpha) * Tau_all_y;
        Tau_L_y = -(1-alpha) * Tau_all_y;
    }*/
    ...
    else
    {
        ...
        /*if(contactMode == 1)
        {
            F_T_L_x_input_dot = -0.04 * (Tau_L_x - l_ft_LPF(3)) - Kl_roll * F_T_L_x_input;
            F_T_L_x_input = F_T_L_x_input + F_T_L_x_input_dot * del_t;
            //F_T_L_x_input = 0;
            F_T_R_x_input_dot = -0.04 * (Tau_R_x - r_ft_LPF(3)) - Kr_roll * F_T_R_x_input;
            F_T_R_x_input = F_T_R_x_input + F_T_R_x_input_dot * del_t;
            //F_T_R_x_input = 0;

            //Pitch 방향
            F_T_L_y_input_dot = 0.04 * (Tau_L_y - l_ft_LPF(4)) - Kl_pitch * F_T_L_y_input;
            F_T_L_y_input = F_T_L_y_input + F_T_L_y_input_dot * del_t;
            //F_T_L_y_input = 0;
            F_T_R_y_input_dot = 0.04 * (Tau_R_y - r_ft_LPF(4)) - Kr_pitch * F_T_R_y_input;
            F_T_R_y_input = F_T_R_y_input + F_T_R_y_input_dot * del_t;
        }
        else if(contactMode == 2)
        {
            F_T_L_x_input_dot = -0.04 * (Tau_L_x - l_ft_LPF(3)) - Kl_roll * F_T_L_x_input;
            F_T_L_x_input = F_T_L_x_input + F_T_L_x_input_dot * del_t;
            F_T_R_x_input_dot = 400.0 * rfoot_rpy_current_(0) - Kr_roll * F_T_R_x_input;
            F_T_R_x_input = F_T_R_x_input + F_T_R_x_input_dot * del_t;
            //F_T_R_x_input = 0;
            ...
            F_T_L_y_input_dot = 0.04 * (Tau_L_y - l_ft_LPF(4)) - Kl_pitch * F_T_L_y_input;
            F_T_L_y_input = F_T_L_y_input + F_T_L_y_input_dot * del_t;
            F_T_R_y_input_dot = -400.0 * rfoot_rpy_current_(1) - Kr_pitch* F_T_R_y_input;
            F_T_R_y_input = F_T_R_y_input + F_T_R_y_input_dot * del_t;
        }
        else
        {
            F_T_L_x_input_dot = 400.0 * lfoot_rpy_current_(0) - Kl_roll * F_T_L_x_input;
            F_T_L_x_input = F_T_L_x_input + F_T_L_x_input_dot * del_t;
            F_T_R_x_input_dot = -0.04 *  (Tau_R_x - r_ft_LPF(3)) - Kr_roll * F_T_R_x_input;
            F_T_R_x_input = F_T_R_x_input + F_T_R_x_input_dot * del_t;
           
            //Pitch 방향
            F_T_L_y_input_dot = -400.0 * lfoot_rpy_current_(1) - Kl_pitch * F_T_L_y_input;
            F_T_L_y_input = F_T_L_y_input + F_T_L_y_input_dot * del_t;
            //F_T_L_y_input = 0;
            F_T_R_y_input_dot = 0.04 *  (Tau_R_y - r_ft_LPF(4)) - Kr_pitch * F_T_R_y_input;
            F_T_R_y_input = F_T_R_y_input + F_T_R_y_input_dot * del_t;
