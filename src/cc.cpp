...
void CustomController::computeSlow()
{
    ...
    if(mpc_cycle <= 94)
    {
        if(LF_matrix(mpc_cycle,2) <= 0.0010 && RF_matrix(mpc_cycle,2) <= 0.0010)
        {
            contactMode = 1;
        ...
        else if(LF_matrix(mpc_cycle,2) <= 0.0010)
        {
            contactMode = 2;
        ...
        else if(RF_matrix(mpc_cycle,2) <= 0.0010)
        {
            contactMode = 3;
    ...
    pinocchio::computeFrameJacobian(model, model_data2, q_pinocchio, RFcframe_id, RFj);//rd_.ee_[1].jac_contact;
    pinocchio::computeFrameJacobian(model, model_data2, q_pinocchio, LFcframe_id, LFj);//rd_.ee_[0].jac_contact;
    ...
    else if(mpc_cycle == 47 || mpc_cycle == 48)
    {
        rd_.ee_[0].contact = 1.0;
        rd_.ee_[1].contact = 1.0;
        com_alpha = DyrosMath::cubic(walking_tick + 40*(mpc_cycle - 47), 0 , 80, 1, 0.5, 0, 0);
        WBC::SetContact(rd_, rd_.ee_[0].contact, rd_.ee_[1].contact);
    }
    else if(mpc_cycle >= 49 && mpc_cycle <= 67)
    {
        rd_.ee_[0].contact = 1.0;
        rd_.ee_[1].contact = 1.0;
        com_alpha = 0.5;
    ...
    else if(mpc_cycle == 69 || mpc_cycle == 70)
    {
        rd_.ee_[0].contact = 1.0;
        rd_.ee_[1].contact = 1.0;
        com_alpha = DyrosMath::cubic(walking_tick + 40*(mpc_cycle - 69), 0 , 80, 0.5, 1, 0, 0);
        WBC::SetContact(rd_, rd_.ee_[0].contact, rd_.ee_[1].contact);
        //WBC::SetContact(rd_, rd_.ee_[0].contact, rd_.ee_[1].contact);
    }
    else if(mpc_cycle > 70 && mpc_cycle < 94)
    {
        rd_.ee_[0].contact = 1.0;
    ...
    if(contactMode == 1)
    {
        ...
        else if(LF_matrix(mpc_cycle, 2) <= 0.0000000001)
    ...
    {
        virtual_temp(0) =  model_data2.oMf[LFcframe_id].translation()(0) - 9.479871019999999704e-02;
    }
    else
    {
    ...
    {  
        ...
        if(contactMode == 1)
        {
            ...
            lbA1(22) = 0.0;//zmpy * nle(2);
            ubA1(22) = 0.0;//zmpy * nle(2);
    ...
    if(mpc_cycle >= 2 && mpc_cycle < controlwalk_time)
    {
        ...
        for (int i = 0; i < MODEL_DOF_VIRTUAL-6; i++)
            ...
            {    
                    for (int i = 0; i < MODEL_DOF_VIRTUAL-6; i++)
                    {   
                        rd_.torque_desired[i] = tau_[i+6] + (rd_.pos_kp_v[i] * (q_pinocchio_desired1[i+7] - rd_.q_[i]) + rd_.pos_kv_v[i] * (qd_pinocchio_desired1[i+6]- rd_.q_dot_[i])); //qd_pinocchio_desired1_prev[i+6]
                        //file[1] << q_pinocchio_desired1[i+7] << " " << rd_.q_[i] << " " ;
                    }
            }
            else
            {   
                for (int i = 0; i < MODEL_DOF_VIRTUAL-6; i++)
                {   
                    rd_.torque_desired[i] = (rd_.pos_kp_v[i] * (q_pinocchio_desired1[i+7] - rd_.q_[i]) + rd_.pos_kv_v[i] * (- rd_.q_dot_[i]));
                    //file[1] << q_pinocchio_desired1[i+7] << " " << rd_.q_[i] << " " ;
                }
            }
    ...
    else if (rd_.tc_.mode == 11)
    {
        ...
        {            
            ...
            if(statemachine.m_shared_memory_int[0] == 1 || statemachine.m_shared_memory_int[0] == 2)
            {
                ...
                if((statemachine.m_shared_memory_int[0] == 1 && walking_tick_stop == true && walking_tick == 0))// || mpc_cycle == 49 || mpc_cycle == 50)
                {   
                    mpc_start_init.m_shared_memory_int[0] = 2;
                    if (walking_tick == 0)
                    { 
                        q_pinocchio_desired = q_pinocchio;
                        q_pinocchio_desired1 = q_pinocchio;
                       
                        walking_tick_stop = false;
                        if (contactMode == 1)// && (mpc_cycle <= 19 || mpc_cycle >= 49))// || (mpc_cycle <= 20 && walking_tick <= 1))
                        {
                            foot_temp(0) = rd_.link_[Right_Foot].xipos(2);
                        ...
                        if(mpc_cycle <= controlwalk_time)
                        {
                            if(upper_on == true && mpc_cycle <= controlwalk_time)
                            {
                                ...
                                if(mpc_cycle <= 48)
                                    std::cout << "mpc_cycle " << " : " << mpc_cycle << " " << walking_tick <<  " " << RF_matrix(mpc_cycle,2) << " " << LF_matrix(mpc_cycle,2) <<std::endl;
                                else
                                    std::cout << "mpc_cycle " << " : " << mpc_cycle << " " << walking_tick <<  " " << RF_matrix_ssp2(mpc_cycle-49,2) << " " << LF_matrix_ssp2(mpc_cycle-49,2) <<std::endl;
                                
                                std::cout << "JOint " << desired_val.m_shared_memory[19] << " " << desired_val.m_shared_memory[20] << std::endl;
                ...
                if ((walking_tick == 0) && (walking_tick_stop == true))
                {
                    ...
                    qd_pinocchio_desired1.setZero();
                    qdd_pinocchio_desired1.setZero();
                    //std::cout << mpc_cycle << " " << walking_tick << "  " << comd(0) << " " << angm(0) <<  " " << qd_pinocchio(0) << " " << qd_pinocchio(13) <<std::endl;
                }
                else
                {
                    KK_temp = 1.0;
                    if(walking_tick_stop == false && mpc_cycle < controlwalk_time)
                    {
                        ...
                        lfoot_ori(0) = 0.3 * (-rfoot_ori_c(0));
                        rfoot_ori(0) = 0.3 * (-rfoot_ori_c(1));
                        lfoot_ori(1) = 0.3 * (-lfoot_ori_c(0));
                        rfoot_ori(1) = 0.3 * (-lfoot_ori_c(1));
                        ...
                        if(mpc_cycle <= 49 && mpc_cycle > 2)
                        {
                            ...
                            if(contactMode == 2)
                            {
                                rfootd1(0) = rfootd1(0)+ 20.0 * (rfoot_mpc(0) -virtual_temp(0) - rd_.link_[Right_Foot].xipos(0) - 0.0378);
                                rfootd1(1) = rfootd1(1)+ 20.0 * (rfoot_mpc(1) -virtual_temp(1)- rd_.link_[Right_Foot].xipos(1));
                                rfootd1(2) = rfootd1(2)+ 20.0 * (rfoot_mpc(2) - rd_.link_[Right_Foot].xipos(2) + foot_temp(0));
                            
                            }
                            else if(contactMode == 3)
                            {
                                lfootd1(0) = lfootd1(0)+ 20.0 * (lfoot_mpc(0) -virtual_temp(0) - rd_.link_[Left_Foot].xipos(0) - 0.0378);
                                lfootd1(1) = lfootd1(1)+ 20.0 * (lfoot_mpc(1) -virtual_temp(1) - rd_.link_[Left_Foot].xipos(1));
                                lfootd1(2) = lfootd1(2)+ 20.0 * (lfoot_mpc(2) - rd_.link_[Left_Foot].xipos(2) + foot_temp(1));
...
void CustomController::momentumControl(RobotData &Robot, Eigen::Vector3d comd,  Eigen::Vector2d ang_, Eigen::Vector3d rfootd, Eigen::Vector3d lfootd, Eigen::Vector2d upperd, Eigen::Vector3d rfootori, Eigen::Vector3d lfootori)
{
    ...
    {   
        ...
        int solved = qp_momentum_control.SolveQPoases(100, qp_result);
