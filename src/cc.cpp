...
CustomController::CustomController(RobotData &rd) : rd_(rd) //, wbc_(dc.wbc_)
{
    ...
    nh.getParam("/tocabi_controller/Zl1x", Zl1x_mpc);
    nh.getParam("/tocabi_controller/Zu1x", Zu1x_mpc);
    nh.getParam("/tocabi_controller/zl1x", zl1x_mpc);
    nh.getParam("/tocabi_controller/zu1x", zu1x_mpc);
    ...
    nh.getParam("/tocabi_controller/Zl1y1", Zl1y_mpc1);
    nh.getParam("/tocabi_controller/Zu1y1", Zu1y_mpc1);
    nh.getParam("/tocabi_controller/zl1y1", zl1y_mpc1);
    nh.getParam("/tocabi_controller/zu1y1", zu1y_mpc1);
    ...
    frame_pos.translation() = rd_.ee_[0].contact_point;
    model2.addBodyFrame("LF_contact", LFjoint_id, frame_pos, LFframe_id);
    frame_pos.translation() = rd_.ee_[1].contact_point;
    ...
    {  
        ...
        state_activations2.push_back(boost::make_shared<ActivationModelQuadraticBarrier>(state_bounds2[i]));//bounds));//state_bounds[i]));
        state_activations3.push_back(boost::make_shared<ActivationModelQuadraticBarrier>(state_bounds3[i]));//bounds));//state_bounds[i]));
        ...
        uRegCost_vector.push_back(boost::make_shared<crocoddyl::CostModelResidual>(
            state,  boost::make_shared<ActivationModelWeightedQuad>(weight_quad_u), boost::make_shared<crocoddyl::ResidualModelControl>(state_vector[i], u_traj_)));//, actuation_vector[i]->get_nu() + 2)));
  ...
  //  runningCostModel_vector[i]->addCost("xReg", xRegCost_vector[i], 1e5);
        runningCostModel_vector[i]->addCost("uReg", uRegCost_vector[i], 1e+10);
       // runningCostModel_vector[i]->addCost("stateReg", stateBoundCost_vector[i], 1e5);
        //runningCostModel_vector[i]->addCost("camReg", camBoundCost_vector[i], 1e5);
        runningCostModel_vector[i]->addCost("comReg", comBoundCost_vector[i], 1e5);
    }    
   ...
   // terminalCostModel->addCost("xReg", xRegCost_vector[N-1], 1e5);
    terminalCostModel->addCost("uReg", uRegCost_vector[N-1], 1e+10);
    //terminalCostModel->addCost("stateReg", stateBoundCost_vector[N-1], 1e3);
    //terminalCostModel->addCost("camReg", camBoundCost_vector[N-1], 1e3);
    terminalCostModel->addCost("comReg", comBoundCost_vector[N-1], 1e5);
    ...
    for(int i = 0; i < N-1; i++)
    {
     ...
     //   runningDAM_data.push_back(
        runningDAM_vector[i]->createData();
    }
    std::cout << "aaaaa" << std::endl;
    problemWithRK4->set_nthreads(2);
...
std::cout << "cccc" << std::endl;
    ...
    {    
        ...
        for(int j = 0 /*xs[i].size()-4*/; j < 19; j ++)
        {
            std::cout << xs[i][j] << ", ";
        }
        std::cout << "qdot " << i << std::endl;
        {
            for(int j = 19 /*xs[i].size()-4*/; j < 37; j ++)
            {
                std::cout << xs[i][j] << ", ";
            }
        }
        ...
        {
            for(int j = 37 /*xs[i].size()-4*/; j < 41; j ++)
            {
                std::cout << xs[i][j] << ", ";
            }
        }
        ...
        if (rd_.tc_.walking_enable == 1.0)
        {
            ...
            if (wlk_on == true && (mpc_cycle > 0 || rd_.tc_.MPC == false))
            {
                ...
                if (rd_.tc_.MPC == true)
                {
                    /*  if(mpcu < 10)
                      {
                          com_mpcx = 0.1 * com_mpcxe + com_mpcx;
                          mpcu = mpcu + 1;
                      }*/
                ...
                {
                    if (rd_.tc_.mom == true && walking_tick > 2000)
                    {
        ...
        if (rd_.tc_.walking_enable == 1.0)
        {
            ...
            if (wlk_on == true && mpc_s == true && mpc_cycle < (t_total * (total_step_num + 1) + t_temp - 1 + 30 * N) / mpct && rd_.tc_.MPC == true && debug == false)
            {
                if (walking_tick == 0 || (walking_tick % mpct == 0 && mpc_cycle * mpct <= walking_tick + mpct))
                {
                    ...
                    if(mpc_cycle < 5)
                    {
                        std::cout << mpc_cycle << "\t" << zmp_mpcx << std::endl;
                    }
                    mom_mpcIx = x11x[3];
                    mom_mpcx = x11x[4];
    ...
    if (mpc_cycle == 540)
    {
        ...
        for (int k = 0; k < 10; k++)
        {
            for (int i = 0; i < 2; i++)
            {
                for (int j = 0; j < 5; j++)
                {
                    printf("%9.5f ", hCx[k][j + 5 * i]);
                }
                printf("\n");
            }
            printf("\n");
            std::cout << "print" << std::endl;
            d_print_mat(ng[1], nx[1], hCx[k], ng[1]);
        }
...
void CustomController::walking_y()
{
    ...
    if (mpc_cycle == 520)
    {
        ...
        for (int i = 0; i < 100; i++)
        {
            ...
            d_ocp_qp_sol_get_sl(i, &qp_soly, sly);
            d_ocp_qp_sol_get_su(i, &qp_soly, suy);
            // file[1] << x11y[0]<<"\t"<< x11y[2]<<"\t" << x11y[3]<<"\t"<< x11y[4]<<"\t"<<suy[0]<<"\t"<<(hd_lgy[i][0] - hCy[i][0] * x11y[0] - hCy[i][2] * x11y[1] - hCy[i][4] * x11y[2] - hCy[i][6] * x11y[3])/hCy[i][8] <<"\t"<<hCy[i][0] * x11y[0] + hCy[i][2] * x11y[1] + hCy[i][4] * x11y[2] + hCy[i][6] * x11y[3] + hCy[i][8] * x11y[4]<<"\t"/*<<hd_lgx[i][0]<<"\t"<<hd_ugx[i][0]<<"\t"*/<<hd_lgy[i][0]-1 * sly[0] << "\t" <<hd_lgy[i][0] + suy[0]<< "\t" <<hd_lgy[i][1]-sly[1]<<"\t"<<hd_lgy[i][1]+suy[1]<<"\t"<<hpipm_statusy<< "\t"<<hCy[i][7]<<std::endl;
        }
    }
...
void CustomController::jointVelocityEstimate()
{
    ...
    if (velEst == false)
    {
        ...
        J_rc.resize(6, MODEL_DOF_VIRTUAL);
        J_lc.resize(6, MODEL_DOF_VIRTUAL);
    }
    ...
    if (velEst == true)
    {
        ...
        int LFcframe_id = model1.getFrameId("LF_contact");
        int RFcframe_id = model1.getFrameId("RF_contact");
        cc_mutex.lock();
        ...
        pinocchio::computeFrameJacobian(model1, model_data1, q_1, LFcframe_id, pinocchio::LOCAL_WORLD_ALIGNED, J_lc);
        pinocchio::computeFrameJacobian(model1, model_data1, q_1, RFcframe_id, pinocchio::LOCAL_WORLD_ALIGNED, J_rc);
...
void CustomController::mpcVariableInit()
{
    ...
    d_zeros(&Zl0x, ns[0], ns[0]);
    d_zeros(&Zu0x, ns[0], ns[0]);
    d_zeros(&zl0x, ns[0], ns[0]);
    d_zeros(&zu0x, ns[0], ns[0]);
    ...
    d_zeros(&Zl0y, ns[0], ns[0]);
    d_zeros(&Zu0y, ns[0], ns[0]);
    d_zeros(&zl0y, ns[0], ns[0]);
    d_zeros(&zu0y, ns[0], ns[0]);
    ...
    d_zeros(&Zl1x, ns[1], ns[1]);
    d_zeros(&Zu1x, ns[1], ns[1]);
    d_zeros(&zl1x, ns[1], 1);
    d_zeros(&zu1x, ns[1], 1);
    d_zeros(&d_ls0y, ns[0], 1);
    d_zeros(&d_us0y, ns[0], 1);
    d_zeros(&Zl1y, ns[1], ns[1]);
    d_zeros(&Zu1y, ns[1], ns[1]);
    d_zeros(&zl1y, ns[1], 1);
    d_zeros(&zu1y, ns[1], 1);
    int_zeros(&idxs1, ns[1], 1);
    d_zeros(&d_ls1x, ns[1], 1);
    d_zeros(&d_us1x, ns[1], 1);
    d_zeros(&ZlNx, ns[N], ns[N]);
    d_zeros(&ZuNx, ns[N], ns[N]);
    d_zeros(&zlNx, ns[N], 1);
    d_zeros(&zuNx, ns[N], 1);
    d_zeros(&d_ls1y, ns[1], 1);
    d_zeros(&d_us1y, ns[1], 1);
    d_zeros(&ZlNy, ns[N], ns[N]);
    d_zeros(&ZuNy, ns[N], ns[N]);
    d_zeros(&zlNy, ns[N], 1);
    d_zeros(&zuNy, ns[N], 1);
    ...
    slx = (double *)malloc(ns[1] * sizeof(double));
    sux = (double *)malloc(ns[1] * sizeof(double));
    x11y = (double *)malloc(nx_max * sizeof(double));
    u11y = (double *)malloc(nx_max * sizeof(double));
    sly = (double *)malloc(ns[1] * sizeof(double));
    suy = (double *)malloc(ns[1] * sizeof(double));
...
void CustomController::momentumControl(RobotData &Robot)
{
    ...
    if (Robot.tc_.MPC == true)
    {
        ...
        H_lower = (Ag_leg * q_dot_est_mu.head(12)).segment<3>(3);
        ...
        H_leg_1 = Ag_leg.block<6, 6>(0, 0) * q_dot_est_mu.head(6);
        H_pitch_l = H_leg_1(4);
        H_leg_1 = Ag_leg.block<6, 6>(0, 6) * q_dot_est_mu.segment<6>(6);
        H_pitch_r = H_leg_1(4);
    }
    else
    {
        ...
        H_roll = H_leg_1(3);
        H_pitch = H_leg_1(4);
        H_leg_1 = Ag_leg.block<6, 6>(0, 0) * rd_.q_dot_virtual_.segment<6>(6);
        H_pitch_l = H_leg_1(4);
        H_leg_1 = Ag_leg.block<6, 6>(0, 6) * rd_.q_dot_virtual_.segment<6>(12);
        H_pitch_r = H_leg_1(4);
